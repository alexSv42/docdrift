...
| --- | --- | --- |
| `data` | array of Project | The page of projects. |
| `nextCursor` | string, nullable | Id to pass as `cursor` to fetch the next page, or `null` if there is no more data. |

```bash
curl "https://api.example.com/v2/projects?limit=50" \
  -H "Authorization: Bearer $API_KEY"
```

## Get a project

`GET /v2/projects/{id}` — returns a single project, or `404` if it does not exist.

```bash
curl https://api.example.com/v2/projects/prj_1 \
  -H "Authorization: Bearer $API_KEY"
```
...
```bash
curl -X PATCH https://api.example.com/v2/projects/prj_1 \
  -H "Authorization: Bearer $API_KEY" \
  -d '{"archived":true}'
```
...
`DELETE /v2/projects/{id}` — returns `204 No Content` on success.

```bash
curl -X DELETE https://api.example.com/v2/projects/prj_1 \
  -H "Authorization: Bearer $API_KEY"
```
